...
To clear the Stache (and refresh any content or settings), you can delete `local/storage/framework/cache/stache` or run `php please clear:stache` via the command line. (Alternatively, you could also delete `local/storage/framework/cache/` or run `php please clear:cache` via the command line. That would clear not just the stache, but also the larger cache, of which the stache is just one part.)
